package cn.com.siss.spring.boot.validate.util;
...
public class ValidationUtils {
     ...
     * @since [产品/模块版本](可选)
     */
    public static <T> void validate(T obj) throws DataException {
        Set<ConstraintViolation<T>> constraintViolations = validator.validate(obj);
        // 抛出检验异常
        if (constraintViolations.size() > 0) {
            throw new DataException(constraintViolations.iterator().next().getMessage());
        }
    }
}
